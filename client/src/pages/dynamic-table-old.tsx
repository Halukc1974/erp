...
export default function DynamicTablePage() {
  ...
  // Use either Supabase data or fallback
  const pageInfo = currentPage || {
    title: fallbackPage!.title,
    href: currentPath,
    pageType: 'table' as const,
    id: 'fallback'
  };
  
  const tableName = currentPage?.id ? 
    tables.find(t => t.pageId === currentPage.id)?.tableName : 
    fallbackPage?.tableName;
  }
